...
export default function Player(props: Props) {
  ...
  if (!track) {
    return (
      <footer className="player-bar">
        ...
        <input type="range" min={0} max={100} value={0} className="seek-bar player-bar-top-seek" readOnly style={{'--p':'0%'} as any} />
        ...
        <div className="player-center">
          <div className="player-controls">
            <button className="icon-btn" disabled><svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polyline points="16 3 21 3 21 8"/><line x1="4" y1="20" x2="21" y2="3"/><polyline points="21 16 21 21 16 21"/><line x1="15" y1="15" x2="21" y2="21"/><line x1="4" y1="4" x2="9" y2="9"/></svg></button>
            <button className="icon-btn" disabled><svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><polygon points="19,20 9,12 19,4"/><line x1="5" y1="19" x2="5" y2="5" stroke="currentColor" strokeWidth="2"/></svg></button>
            <button className="play-btn" disabled><svg width="18" height="18" fill="currentColor" viewBox="0 0 24 24"><polygon points="5,3 19,12 5,21"/></svg></button>
            <button className="icon-btn" disabled><svg width="24" height="24" fill="currentColor" viewBox="0 0 24 24"><polygon points="5,4 15,12 5,20"/><line x1="19" y1="5" x2="19" y2="19" stroke="currentColor" strokeWidth="2"/></svg></button>
            <button className="icon-btn" disabled><svg width="20" height="20" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24"><polyline points="17 1 21 5 17 9"/><path d="M3 11V9a4 4 0 0 1 4-4h14"/><polyline points="7 23 3 19 7 15"/><path d="M21 13v2a4 4 0 0 1-4 4H3"/></svg></button>
          </div>
        </div>
  ...
  return (
    <footer className="player-bar">
      ...
      {/* Thin full-width seek bar pinned to the top edge of the whole bar
          (YT Music desktop layout) — replaces the old inline seek-row that
          used to sit in the center column next to the transport controls. */}
      <input type="range" min={0} max={duration || 100} value={currentTime}
        className="seek-bar player-bar-top-seek" style={{'--p':`${progress}%`} as React.CSSProperties}
        onChange={e => handleSeek(Number(e.target.value))} />

      {/* LEFT: track info — clicking anywhere here (not the like button)
          expands to the full now-playing view, same as clicking the card
          in YT Music; there's no separate dedicated expand button anymore. */}
      <div className="player-left" onClick={props.onToggleExpanded} title="Perbesar" style={{cursor:'pointer'}}>
        ...
        <button className={`icon-btn ${liked?'active':''}`}
          onClick={e => { e.stopPropagation(); props.onProfileChange(toggleLike(props.profile, track.id)); }} title="Sukai">
      ...
      {/* CENTER: controls — bigger now that the seek row moved out of this column */}
      <div className="player-center">
        <div className="player-controls">
      ...
      {/* RIGHT: utilities — just volume now; queue/lyrics panel and the
          separate expand button are gone, both subsumed by ExpandedPlayer
          (opened via the player-left click above). */}
      <div className="player-right">
